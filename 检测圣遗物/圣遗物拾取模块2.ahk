...
; ---------- 可调参数 ----------
global AP_IDLE_INTERVAL := 120     ; 空闲时检测间隔(没列表,ms)
global AP_ACTIVE_INTERVAL := 50    ; 检测到列表后的快速间隔(ms)
...
global AP_PICK_WAIT := 70          ; 按F后等待(ms)
...
AP_ProcessPickup() {
    global
    ; 先整列表OCR一次，确认有没有圣遗物（没有就不动，避免空滚）
    if !AP_HasAnyArtifact()
        return

    ; 有圣遗物，逐格遍历可见列表（最多滚7次覆盖6条+余量）
    Loop 7 {
        ; 超时保护
        ...
        ; OCR当前F框指向的这一行（先短暂等画面稳定，避免滚动残影导致OCR认错）
        Sleep 10
        isArt := AP_IsArtifactAtRow(fy)
        if !isArt {
            ; 可能滚动刚停画面没稳，再等一下重测一次，避免误判把圣遗物滚走
            Sleep 10
            isArt := AP_IsArtifactAtRow(fy)
        }
        if isArt {
            ...
            Sleep AP_PICK_WAIT
            ; 拾取后该条消失、列表刷新，F位置会变。
            ; 重新确认列表里是否还有圣遗物，没有就结束
            if !AP_HasAnyArtifact()
                return
            ; 还有圣遗物，继续遍历（不滚，先测当前F行——可能下一条补上来正好是圣遗物）
            continue
        }
